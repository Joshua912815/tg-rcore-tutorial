//! # 第三章观测版：多道程序与分时多任务
//!
//! 这个版本围绕 T2L21 组织，目标不是增加新的内核机制，而是让第三章的关键状态
//! 以一种更容易复现、记录和解释的方式暴露出来。
//!
//! ## 核心概念
//!
...
//! - **状态观测**：统一的状态快照、断点事件、panic breadcrumb 调用链
//!
//! 教程阅读建议：
//!
//! - 先看 `observe` 模块：理解“我们到底在记录哪些关键状态”；
//! - 再看 `rust_main` 主循环：理解正常、异常、断点和崩溃路径如何产生日志；
//! - 最后看 `Trace` 的扩展请求：理解如何在特定执行点主动转储当前状态。
...
#[inline(never)]
fn panic_stage2(task_id: usize, marker: usize) -> ! {
    let _scope = observe::scope("panic_stage2");
    panic!("T21 crash demo task={task_id} marker={marker}");
}

#[inline(never)]
fn panic_stage1(task_id: usize, marker: usize) -> ! {
    let _scope = observe::scope("panic_stage1");
    panic_stage2(task_id, marker)
}
...
extern "C" fn rust_main() -> ! {
    let _scope = observe::scope("rust_main");
    ...
    for (i, app) in tg_linker::AppMeta::locate().iter().enumerate() {
        ...
        observe::init_task(i, entry, tcbs[i].sp());
        index_mod += 1;
    }
    println!();
    ...
    while remain > 0 {
        ...
        if !tcb.finish {
            loop {
                let _scope = observe::scope("schedule_loop");
                ...
                observe::on_enter(i, tcb.sepc(), first_run[i]);
                first_run[i] = false;
                ...
                let scause_bits = scause_reg.bits();
                let sepc = tcb.sepc();
                let stval = stval::read();
                let finish = match scause_reg.cause() {
                    // ─── 时钟中断：时间片用完，切换到下一个任务 ───
                    Trap::Interrupt(Interrupt::SupervisorTimer) => {
                        // 清除时钟中断（设置为最大值，避免立即再次触发）
                        tg_sbi::set_timer(u64::MAX);
                        observe::on_timer(i, sepc);
                        log::trace!("app{i} timeout");
                        false // 不结束任务，切换到下一个
                    }
                    // ─── ebreak：在特定执行点停下并输出快照，然后继续 ───
                    Trap::Exception(Exception::Breakpoint) => {
                        let inst16 = unsafe { (sepc as *const u16).read_volatile() };
                        let step = if inst16 & 0b11 == 0b11 { 4 } else { 2 };
                        observe::on_breakpoint(i, sepc);
                        observe::dump_task(i, "ebreak");
                        println!("[OBS][breakpoint] task={i} step={step} inst16={inst16:#x}");
                        tcb.advance_pc(step);
                        continue;
                    }
                    ...
                    Trap::Exception(e) => {
                        observe::on_exception(i, trap_name(e), scause_bits, sepc, stval);
                        log::error!("app{i} was killed by {e:?}");
                        true
                    }
                    // ─── 未预期的中断：杀死应用 ───
                    Trap::Interrupt(ir) => {
                        observe::on_exception(i, "unexpected-interrupt", scause_bits, sepc, stval);
                ...
                if finish {
                    observe::dump_task(i, "task-finished");
                    tcb.finish = true;
                    remain -= 1;
                }
                break;
    ...
    observe::dump_summary(index_mod);
    // 所有用户程序执行完毕，关机
...
fn panic(info: &core::panic::PanicInfo) -> ! {
    if let Some(task_id) = task::current_task_id() {
        observe::dump_task(task_id, "panic");
    }
    observe::dump_breadcrumbs();
    println!("{info}");
    tg_sbi::shutdown(true)
}

fn trap_name(e: scause::Exception) -> &'static str {
...
/// 各依赖库所需接口的具体实现
mod impls {
    use crate::{observe, panic_stage1, task};
    use tg_syscall::*;
    ...
    impl Trace for SyscallContext {
        ...
        fn trace(&self, _caller: Caller, trace_request: usize, id: usize, data: usize) -> isize {
            let _scope = observe::scope("trace_syscall");
            match trace_request {
                ...
                3 => {
                    if let Some(task_id) = task::current_task_id() {
                        observe::on_trace_request(task_id, trace_request);
                        observe::dump_task(task_id, "trace-snapshot");
                        0
                    } else {
                        -1
                    }
                ...
                4 => {
                    if let Some(task_id) = task::current_task_id() {
                        observe::on_trace_request(task_id, trace_request);
                        observe::dump_task(task_id, "trace-crash");
                        panic_stage1(task_id, data)
                    } else {
                        -1
                    }
                }
