...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=CARGO_FEATURE_CRASH_DEMO");
...
fn build_apps() {
    ...
    let case_key = if env::var("CARGO_FEATURE_CRASH_DEMO").is_ok() {
        "ch3_observe_crash"
    } else if env::var("CARGO_FEATURE_EXERCISE").is_ok() {
        "ch3_observe_exercise"
    } else {
        "ch3_observe"
    };
...
fn ensure_tg_user() -> PathBuf {
    ...
    // 与章节 crate 并列放置在仓库根目录时，优先复用根目录已有的用户态工程
    if let Some(parent) = manifest_dir.parent() {
        let sibling = parent.join(&local_dir_name);
        if sibling.join("Cargo.toml").exists() {
            ensure_workspace_table(&sibling);
            return sibling;
        }
    }
