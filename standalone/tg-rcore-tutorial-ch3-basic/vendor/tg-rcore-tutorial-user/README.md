# tg-rcore-tutorial-user (vendored subset)

This vendored directory contains only the user-space programs required to reproduce
the standalone Chapter 3 lab crate. It is packaged with the published crate so
that `cargo run` and `./test.sh` do not depend on any parent workspace layout.
