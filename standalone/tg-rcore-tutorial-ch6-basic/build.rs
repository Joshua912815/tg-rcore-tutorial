...
fn ensure_tg_user() -> PathBuf {
    let manifest_dir = PathBuf::from(env::var_os("CARGO_MANIFEST_DIR").unwrap());
    let bundled_user_dir = manifest_dir.join("vendor").join("tg-rcore-tutorial-user");
    if bundled_user_dir.join("Cargo.toml").exists() {
        ensure_workspace_table(&bundled_user_dir);
